...
from jamasp.models.relationship import Relationship
from jamasp.models.source import DataSource
from jamasp.models.user import User
from jamasp.schemas.knowledge import (
    SCHEMA_VERSION,
    Bilingual,
    KnowledgeEntity,
    KnowledgeExport,
...
async def knowledge_payload(source_id: UUID, session: AsyncSession) -> dict[str, Any]:
    """The knowledge export as plain data, for callers with no HTTP request.

    The background report worker needs exactly what the endpoint returns, but has
    no authenticated user to satisfy the dependency.
    """
    export = await _build_export(source_id, session)
...
async def export_knowledge(
    ...
    return await _build_export(source_id, session)


async def _build_export(source_id: UUID, session: AsyncSession) -> KnowledgeExport:
